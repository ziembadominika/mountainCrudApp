...
public class ContinentTest {
    ...
    public void shouldGetMountainsFromContinent(){
        ...
        List<MountainRange>slovakMountains=new ArrayList<>();
        slovakMountains.add(tatraMountains);

        List<Country> europeCountries = new ArrayList<>();
        Country poland = new Country(1L, "Poland", polishMountains);
        Country slovakia = new Country(2L, "Slovakia", slovakMountains);
        europeCountries.add(poland);
        europeCountries.add(slovakia);

        Continent europe = new Continent(1L, "Europe", europeCountries);

        //When
        List<Mountain> europeMountainsList = europe.getMountainFromContinent("Europe");

        //Then
        Assert.assertEquals(4, europeMountainsList.size());
    }
